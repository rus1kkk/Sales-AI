...
const InputSchemas = {
  nameInput: z
    ...
    .min(1, 'Введите имя')
    .refine(
      (val) => val.length === 0 || /^[А-ЯЁа-яё\s-]+$/.test(val),
      'Допускается только кириллица, пробелы и дефисы',
    ),
  ...
  emailInput: z
    ...
    .refine(
      ...
      'Email не может содержать две точки подряд',
    )
  ...
  phoneInput: z
    ...
    .refine(
      (val) => val.length === 0 || /^[+]?[0-9]*$/.test(val),
      'Разрешены только цифры и + в начале',
    ),

  passwordDebounced: z.string().min(1, 'Введите пароль'),
}
...
const DebouncedInputSchemas = {
  ...
  emailDebounced: z.string().email('Некорректный формат email'),
  ...
  passwordDebounced: z
    ...
    .refine(
      (val) => val.length === 0 || /[A-ZА-ЯЁ]/.test(val),
...
const BlurSchemas = {
  ...
  emailBlur: z.string().min(1, 'Введите email').email('Некорректный формат email'),
  ...
  passwordLoginBlur: z.string().min(1, 'Введите пароль'),
}
